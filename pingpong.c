...
void initialiser(balls *B,surf *player,surf *compt,score_pingpong *scoreplayer,score_pingpong *scorecom,surf *net  )
{
       ...
       net->img=IMG_Load("graphic/stages/ping pong_1000_800.png"); //net separation
       net->position.x=465;
       net->position.y=0;
...
SDL_FreeSurface(B.ball);
SDL_FreeSurface(player.img);
SDL_FreeSurface(compt.img);
SDL_FreeSurface(net.img);
SDL_FreeSurface(scoreplayer.text);
SDL_FreeSurface(scorecom.text);
